...
//burnout 3 signature key (all regions)
const unsigned char signature_key[0x10] = { 0x8F, 0x4B, 0xC9, 0xB5, 0xD6, 0xB0, 0x1E, 0x7F, 0x47, 0x72, 0xC5, 0xAC, 0xE0, 0x4B, 0xC9, 0x22 };

int main(int argc, char* argv[])
{
	SetConsoleTitle(TEXT("Burnout 3 (XBOX) Hash Fix"));
	...
	printf("Burnout 3 (XBOX)\nHash Fix by feudalnate (December 21st, 2019)\
	\n----------------------------------------\n\n");

	//check args
	if (argc != 3)
	{
		printf("Usage: b3fix <XboxHDKey> <input file>");
		goto exit;
	}
	...
	{
		printf("Error: Invalid XboxHDKey. Expected 32 character hexadecimal string", filename);
		goto exit;
	}
	...
	if (xboxhdkey == NULL)
	{
		...
		printf("Error: Invalid XboxHDKey. Failed to convert from hexadecimal", filename);
		goto exit;
	}
	...
	//sign data
	unsigned char* hash = CalculateNonRoamable(&signature_key, xboxhdkey, buffer, (0xEA00 - 0x14));
	memcpy(buffer + (0xEA00 - 0x14), hash, 0x14);
